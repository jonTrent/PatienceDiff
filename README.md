...
The PatienceDiffPlus algorithm is a new algorithm that extends the PatienceDiff algorithm, to include the *likely* lines that moved.  The algorithm is elegantly straightforward.  PatienceDiffPlus takes the original lines deleted from aLines[] along with the lines inserted into bLines[], and runs these residual subset of lines through the PatienceDiff algorithm.  If any lines match between in this subset, then they are *assumed* to have moved rather than being independent deletions and insertions.  The results of the initial PatienceDiff algorithm are then adjusted to record the likely moves.  This is repeated, with any remaining deletions and insertions again run through the PatienceDiff algorithm, until no more matches are found.
